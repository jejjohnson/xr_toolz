...
- **Tier C — Layer 1 (Operators)**: `Dataset → Dataset` (with the documented viz exception per D10). Operators select variables via constructor args, then delegate to Tier B. Multi-input operators (metrics) take multiple Datasets.
...
- The Operator contract stays clean: every Operator takes Dataset(s) and produces a Dataset (or a Figure for terminal viz). `Sequential` and `Graph` only ever see Tier C, so the composition surface is uniform across modules.
